...
#define LLVM_CLASS_OF(_id_name_)                                               \
 ...
 classof(const ASTNode*                                                        \
             _id_name_ /* NOLINT(bugprone-macro-parentheses) */) noexcept      \
     -> bool
...
class VarAssignExprAST : public ExprAST {
 ...
 public:
  ...
  VarAssignExprAST(
      std::vector<VarAssignPair> VarAs, std::unique_ptr<ExprAST> Body
  ) noexcept
      : ExprAST(Kind)
      , VarAs(std::move(VarAs))
      , Body(std::move(Body)) {}
...
class PrototypeAST : public ASTNode {
 ...
 protected:
  PrototypeAST(
      ASTNodeKind K, std::string Name, std::vector<std::string> Args
  ) noexcept
      : ASTNode(K)
      , Name(std::move(Name))
      , Args(std::move(Args)) {}
...
class ProtoBinaryAST : public PrototypeAST {
 ...
 public:
  ...
  ProtoBinaryAST(
      char Op, std::array<std::string, 2> Args, int Precedence
  ) noexcept
      : PrototypeAST(
          Kind,
          fmt::format(FMT_COMPILE("binary{}"), Op),
...
class ProtoUnaryAST : public PrototypeAST {
 public:
  ...
  ProtoUnaryAST(char Op, std::array<std::string, 1> Args) noexcept
      : PrototypeAST(
          Kind,
          fmt::format(FMT_COMPILE("unary{}"), Op),
          {std::make_move_iterator(Args.begin()),
           std::make_move_iterator(Args.end())}
      ) {}
